...
module.exports = {
  up: async (queryInterface, Sequelize) => {
     ...
     * }], {});
     */
    const file = fs.readFileSync("/clinical/data/circa.pheno.json");
    ...
    for (const patient of patients) {
      ...
      await session.run("MERGE (a:Cohort {cohort: 'CIRCA'}) RETURN a");
      ...
      try {
        await session.run(
          "MERGE (a:Patient {cohort: 'CIRCA'," + clinicalInfo + "}) RETURN a"
        );
